...
const DeleteNote = (props) => {
  ...
  useEffect(() => {
    const result =
      data !== undefined && data?.Notes.filter((x) => x.isTrash === true);
    setTrash(result);
  ...
  const currentItems = Trash.slice(startIndex, endIndex);
    ...
    <div className="w-full flex min-h-screen md:h-auto ">
      ...
      {Trash !== undefined && Trash?.length !== 0 ? (
        <div className="md:w-[80%] w-full">
          <div className="w-[95%]  h-auto mx-auto bg-white dark:bg-slate-900 rounded-lg dark:text-white mt-10 border-2">
            ...
            <div className="flex flex-wrap mx-2.5">
              {currentItems.map((item) => TrashCard(item))}
            </div>
        ...
        <div className=" md:w-[80%] mx-auto ">
          <div className="md:w-[95%] w-full p-2 flex items-center justify-center text-2xl font-semibold h-[250px] mx-auto bg-white dark:bg-slate-900 rounded-lg dark:text-white mt-10 border-2">
            Trash Component is Empty 
          </div>
        </div>
      )}
    </div>
